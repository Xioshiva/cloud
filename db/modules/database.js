...
/* En fonction de "user", on ajoute le nouveau succes
 * retourne "true" si tout s'est bien passé pour addapter le code http de retour de la route 
 */
exports.addSucces = function(user, succes){
    // On ne s'occupe plus que du fichier "succes.json"
    let file = 'db/succes.json';
    // lecture du fichier
    fs.readFile(file, function (err, data){
        if(err) throw err;
        // parsing json
        obj = JSON.parse(data);
        // on parcours tous les users
        for(let i = 0; i < obj.content.length; i++){
            if(obj.content[i].user == user){
                // quand on tombe sur le bon, on ajoute le nouveau succes
                obj.content[i].succes.push({value:succes});
            }
        }
        // écriture du nouvel objet
        fs.writeFile(file, JSON.stringify(obj), (err) => {
...
exports.getSucces = function(user){
    ...
    for(let i = 0; i < obj.content.length; i++){
        if(obj.content[i].user == user){
            // quand on tombe sur le bon on retourne la liste de ses succes favoris
            return obj.content[i].succes;
        }
    }
    // si le user n'est pas présent dans la db
    return false;
}
